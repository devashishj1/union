...
class ProcurementWorkflow:
    ...
    async def _run_analysis_and_store_result(self, user_id: str, assistant_id: str) -> str:
        session = self.sessions[user_id]
        data_for_analysis = {
        ...
        # Create a thread with the analysis request as the user message.
        thread = await openai.beta.threads.create({
            "messages": [
                {
                    "role": "user",
                    "content": json.dumps(data_for_analysis)  # or format as needed
                },
            ],
        })

        # Start the run, passing in the assistant_id.
        run = await openai.beta.threads.runs.createAndPoll(thread["id"], {
            "assistant_id": assistant_id,
        })

        # Retrieve the messages from the thread run.
        messages_response = await openai.beta.threads.messages.list(thread["id"], {
            "run_id": run["id"],
        })

        # Process the messages to obtain the final analysis.
        # For instance, you might assume the last message contains the result.
        final_analysis = messages_response["messages"][-1]["content"]

        session["history"].append(f"System: {final_analysis}")

        # Store final analysis and data for future reference.
